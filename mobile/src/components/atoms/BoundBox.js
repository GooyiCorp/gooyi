...
export default function BoundBox({boxHeight, boxWidth, top, left}) {
    const cornerHeight = boxHeight/4 <= 30 ? boxHeight/4 : 30
    const cornerRadius = Math.floor(boxHeight/20) >= 5 ? Math.floor(boxHeight/20) : 5
  return (
    <View style={{position: 'absolute', height: boxHeight, width: boxWidth, top: top, left: left, transform: [{scale: 1.1}]}}>
