...
import { generate_coupon_code, sendError, sendServerError, sendSuccess } from '../../helper/client.js';
...
couponRoute.get('/', verifyToken, async (req, res) => {
    ...
    try {
        const user = await prisma.user.findUnique({where: {user_id}, include: {coupons: true}});
