  ...
  try {
    const store = await prisma.store.findUnique({
      ...
      include: {
        ...
        status: true,
        service: true,
        _count: { select: { FavoritedUsers: true } },
      },
    });
...
storeRoute.get("/loggedin/info/:id", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const user = await prisma.user.findUnique({
      where: { user_id },
      select: { UserPoints: { where: { store_id } } },
    });
    var point = 0;
...
storeRoute.get("/", async (req, res) => {
  ...
  try {
    let stores = await prisma.store.findClosestStores({
      longitude,
      latitude,
...
storeRoute.get("/find", verifyToken, async (req, res) => {
  ...
  try {
    const user = await prisma.user.findUnique({
      where: { user_id },
      include: { FavoriteStores: { select: { store_id: true } } },
    });
...
storeRoute.get("/search", async (req, res) => {
  const error = find_stores_validate(req.query);
  if (error) return sendError(res, error);
  const { longitude, latitude, radius, searchString, category, sort } =
    req.query;
...
storeRoute.post("/feedback", verifyToken, async (req, res) => {
  ...
  if (!text || !store_id)
    return sendError(res, "text and store_id is required");
  const user_id = req.user.id;
  ...
  try {
    ...
    if (user.FeedBacks.length > 0) {
      ...
      if (last_feedback_date === today)
        return sendError(res, "One Feedback per day", 403);
    }
    await prisma.user.update({
      where: { user_id },
      data: {
