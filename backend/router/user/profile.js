...
profileRoute.post("/register", async (req, res) => {
  const { first_name, last_name, email, phone, key } = req.body;
...
profileRoute.get("/login-redirect", async (req, res) => {
  ...
  if (err) return sendError(res, err);
  const { exp, accessToken, refreshToken } = req.query;
  try {
    ...
    if (now - exp >= 600000)
      return res.send(
        render(redirect_page, { redirect_link: link + "?error=expired" }),
      );
    if (refreshToken in TOKEN_LIST)
      return res.send(
        render(redirect_page, { redirect_link: link + "?error=used" }),
      );
    try {
      jwt.verify(accessToken, process.env.JWT_SECRET_KEY, { complete: true });
    } catch (e) {
      return sendError(res, "jwt expired. Please try again!");
    }
...
profileRoute.get("/register-redirect", async (req, res) => {
  ...
  try {
    ...
    return res.send(
      render(redirect_page, {
        redirect_link: link + `?email=${email}&key=${key}`,
      }),
    );
  } catch (err) {
    logger.error(err);
